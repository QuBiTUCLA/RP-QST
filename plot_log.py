#Used to plot entropy and generator/discriminator loss from the output.csv's
...
for i in range(len(depth)):
    for j in range(len(epochs)):
        for k in range(len(batches)):
            #for mo in range(len(model)):
                name = str(depth[i]) + '_' + str(num_qubits) + '_' + str(epochs[j]) + '_' + str(batches[k]) + '_' + str(bound)# + '_' + str(model[mo])
                ...
                if os.path.exists(desired_folder):
                    ...
                    plt.title("Progress in the loss function - " + name)
                    ...
                    plt.ylabel('loss')
                    plt.show()
                    #plt.savefig('logs/Generator_And_Discriminator_Loss_Plots/'+name+'_loss.png')

                    # Plot progress w.r.t relative entropy
                    plt.figure(figsize=(6, 5))
                    plt.title("Relative Entropy - " + name)
                    plt.plot(epoch, entropy, color='mediumblue', lw=4, ls=':')
                    plt.grid()
                    plt.xlabel('time steps')
                    plt.ylabel('relative entropy')
                    plt.show()
                    #plt.savefig('logs/Entropy_Plots/'+name+'_entr.png')
